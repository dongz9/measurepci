#include "main.h"
#define MAX_CLT_TX_BURST 32
#define MAX_CLT_RX_BURST 32
...
void run_client(int client_id, struct rte_mempool **l2fwd_pktmbuf_pool)
{
	...
	LL nb_tx_try = 0, nb_tx_succ = 0, nb_tx_fail = 0, nb_rx = 0;
	...
	while (1) {
		...
		for(i = 0; i < MAX_CLT_TX_BURST; i ++) {
			...
			tx_pkts_burst[i]->pkt.pkt_len = 40;
			tx_pkts_burst[i]->pkt.data_len = 40;
		...
		nb_tx_try += MAX_CLT_TX_BURST;
		nb_tx_succ += nb_tx_new;
		nb_tx_fail += (MAX_CLT_TX_BURST - nb_tx_new);
		...
		if (unlikely(nb_tx_succ >= 2000000)) {
			cur_tsc = rte_rdtsc();
			double nanoseconds = C_FAC * (cur_tsc - prev_tsc);
			double seconds = nanoseconds / GHZ_CPS;
			prev_tsc = cur_tsc;

			printf("Lcore %d: TX try = %.2f, TX succ = %.2f, TX fail = %.2f, latency = %.2f us, sleep = %.2f\n"
				"\tnb_rx = %lld, magic passed = %lld\n",
				lcore_id, nb_tx_try / seconds, nb_tx_succ / seconds, nb_tx_fail / seconds,
